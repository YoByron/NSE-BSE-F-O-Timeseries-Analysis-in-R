...
```{python optionschain_p, echo=FALSE, ,echo=FALSE, error=FALSE, message=FALSE, warning=FALSE,tidy=TRUE}
...
library(reticulate)
```
## Nifty `r py$nf_ul` `py$nf_nearest`

## BankNifty `r py$bnf_ul` `py$bnf_nearest`

### Major Support in Nifty: `r py$nf_highestoi_CE`

### Major Resistance in Nifty: `r py$nf_highestoi_PE`

### Major Support in BankNifty: `r py$bnf_highestoi_CE`

### Major Resistance in BankNifty: `r py$bnf_highestoi_PE`
...
```{r jugaad-data_r, include=TRUE, echo=FALSE, tidy=TRUE}
py$symbol <- "INFY"
```

## <span style='color: #1d7874'> `r py$symbol` Stock Price Chart

```{python jugaad-data_p, include=TRUE, echo=FALSE, tidy=TRUE}
