...
```{python optionschain_p, error=FALSE, message=FALSE, warning=FALSE, ,echo=FALSE, include=FALSE, tidy=TRUE}
...
#If having trouble in installing, copy and paste the lines in the console
packages <- function()
{
  # Package names
packages <- c("ggplot2", 
...
```{r blackscholes, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
...
| Calculator                 | Value                       |
|----------------------------|-----------------------------|
| 50% Profit                 | `r print(0.05*30)`          |
| 30% Loss                   | `r print(0.03*30)`          |
| Profitability              | `r print(paste("45","%"))`  |


```{r nse_general, error=FALSE, message=FALSE, warning=FALSE, include=FALSE}
#setrepositories()

#no package called nser
...
# install.packages("devtools")
#install rtools
# devtools::install_github("rsquaredacademy/nse2r",force=TRUE)

#no package called nsepy
...
```{r nifty_pe_r, error=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results='asis', tidy=TRUE}
pythonr <- py$nifty_pe
knitr::kable(pythonr)%>%
  kable_styling(bootstrap_options = "condensed",
                , full_width = F
...
## <span style='color: #bf6900'> `r py$symbol` Stock Price Chart
...
```
## R version
```{r jugaad-data_r, echo=FALSE, tidy=TRUE, include=FALSE}
## Visualize
library(reticulate)
library(tidyverse)
library(tidyquant) 
...
pydf %>%
    ...
    geom_line() +
    labs(title = paste(py$symbol,"Line Chart"), y = "Closing Price", x = "Time") + 
    theme_tq()
```

## Stock Portfolio

```{r stock_portfolio, eval=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=FALSE}
#Dyoptions not found Error
